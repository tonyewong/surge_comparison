...
covariate_proj <- covariates[match(years_proj, covariates[,'year']), covar_name]
...
rl <- vector('list', nsite); names(rl) <- site_names
for (dd in 1:nsite) {
  rl[[dd]] <- array(dim=c(nrp, nmodel, nyear), dimnames=list(return_periods, 1:nmodel, years_proj))
}
...
for (dd in 1:nsite) {
  ...
  for (mm in 1:nmodel) {
    ...
    for (yy in 1:nyear) {
      ...
      for (rr in 1:nrp) {
        rl[[dd]][rr,mm,yy] <- qevd(1-1/return_periods[rr], loc=mu, scale=sigma, shape=xi)
      }
    }
  }
}
...
save(list=c('rl', 'site_names', 'covariates', 'return_periods', 'years_proj',
            'nsite', 'nmodel', 'nrp', 'nyear', 'covariate_proj', 'sim_id'), file=filename.returnlevels)
